...
subprocess.run(f"../build/embed {img.shape[1]} {img.shape[0]} {wmlen}".split())
...
gpuout = np.fromfile('../out/gpuout.bin', dtype=np.float32).reshape(img_shape)
img_YUV[:, :, 0] = gpuout
embed_img = cv2.cvtColor(img_YUV, cv2.COLOR_YUV2BGR)
cv2.imwrite("../out/gpuout.png", embed_img)
